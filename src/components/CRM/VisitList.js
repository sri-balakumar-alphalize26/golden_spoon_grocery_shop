  ...
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={onPress} style={styles.itemContainer}>
      <View style={styles.leftColumn}>
        <Text style={styles.head}>{item?.customer?.name?.trim() || '-'}</Text>
        <View style={styles.rightColumn}>
          <Text style={styles.content}>{item?.employee?.name || '-'}</Text>
          <Text style={[styles.contentRight]}>{formatDate(item?.date_time, 'dd MMM yyyy HH:mm:ss ') || '-'}</Text>
        </View>
      </View>
      <View style={styles.rightColumn}>
        <Text style={styles.content}>{item?.site_location?.site_location_name}</Text>
        {/* <Text style={styles.contentRight}>{{visitPurpose}}</Text> */}
      </View>
    </TouchableOpacity>
  );
};
