...
const MarketStudyList = ({ item, onPress }) => {
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={onPress} style={styles.itemContainer}>
      <View style={styles.leftColumn}>
        ...
        <View style={styles.rightColumn}>
          <Text style={styles.content}>{item?.contact_number || '-'}</Text>
          <Text style={[styles.contentRight, {color: 'red'}]}>{item?.industry_type || '-'}</Text>
        </View>
      </View>
      <View style={styles.rightColumn}>
...
export default MarketStudyList;
