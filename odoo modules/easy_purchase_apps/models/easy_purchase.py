...
class EasyPurchase(models.Model):
    _name = 'easy.purchase.app'
    _description = 'Easy Purchase Entry (App)'
    ...
    line_ids = fields.One2many('easy.purchase.line.app', 'purchase_id', string='Purchase Lines', copy=True)
    ...
    payment_ids = fields.Many2many(
        'account.payment',
        'easy_purchase_app_payment_rel',
        'easy_purchase_app_id',
        'payment_id',
    ...
    payment_method_id = fields.Many2one(
        'easy.purchase.payment.method.app',
        string='Payment Method',
        required=True,
        domain="[('company_id', '=', company_id)]",
        default=lambda self: self.env['easy.purchase.payment.method.app'].get_default_payment_method(),
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('easy.purchase.app') or _('New')
...
class EasyPurchaseLine(models.Model):
    _name = 'easy.purchase.line.app'
    _description = 'Easy Purchase Line (App)'
    _order = 'sequence, id'
    ...
    purchase_id = fields.Many2one('easy.purchase.app', string='Purchase', required=True, ondelete='cascade')
    ...
    def action_print_barcode(self):
        self.ensure_one()
        wizard = self.env['easy.purchase.barcode.wizard.app'].create({
            'line_id': self.id,
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'easy.purchase.barcode.wizard.app',
            'res_id': wizard.id,
            'view_mode': 'form',
            'target': 'new',
        }
