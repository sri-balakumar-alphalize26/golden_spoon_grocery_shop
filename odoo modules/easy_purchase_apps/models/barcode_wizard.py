...
class EasyPurchaseBarcodeWizard(models.TransientModel):
    _name = 'easy.purchase.barcode.wizard.app'
    _description = 'Easy Purchase Barcode Print Wizard (App)'

    line_id = fields.Many2one('easy.purchase.line.app', string='Purchase Line')
    ...
    def action_print(self):
        ...
        return self.env.ref('easy_purchase_apps.action_report_easy_purchase_barcode_app').report_action(self)


class EasyPurchaseBarcodeReport(models.AbstractModel):
    _name = 'report.easy_purchase_apps.report_barcode_label'
    _description = 'Easy Purchase Barcode Label Report (App)'

    @api.model
    def _get_report_values(self, docids, data=None):
        wizard = self.env['easy.purchase.barcode.wizard.app'].browse(docids)
        products = []
        quantity = 1
        company_name = ''
        label_size = '38x25'
        ...
        return {
            'doc_ids': docids,
            'doc_model': 'easy.purchase.barcode.wizard.app',
            'docs': wizard,
            'products': products,
            'quantity': quantity,
