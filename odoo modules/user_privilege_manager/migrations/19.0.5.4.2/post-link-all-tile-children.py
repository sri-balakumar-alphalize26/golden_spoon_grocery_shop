...
# Each entry: parent xmlid -> tuple of child xmlids whose parent_id should
# point at the parent. Same shape lets us add more tiles in one place.
LINKS = {
...
def migrate(cr, version):
    ...
    Same approach as the v19.0.5.4.1 Products migration but generalised --
    one big mapping table covers every tile that has nested per-operation
    gates. Idempotent: if a child is already linked to the right parent we
    log and skip; if a child row doesn't exist yet (seed not loaded for
    some reason) we log a WARNING and continue.
    """
    env = api.Environment(cr, SUPERUSER_ID, {})
    for parent_xmlid, child_xmlids in LINKS.items():
        parent = env.ref(
            'user_privilege_manager.%s' % parent_xmlid,
