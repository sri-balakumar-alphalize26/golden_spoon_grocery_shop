...
class PrivilegeBulkCreateWizard(models.TransientModel):
    _name = 'privilege.bulk.create.wizard.app'
    _description = 'Bulk Create User Privileges (Apps)'

    user_ids = fields.Many2many(
        'res.users',
        'privilege_bulk_wizard_app_user_rel',
        'wizard_id', 'user_id',
        string='Users',
        domain=[('share', '=', False)],
        required=True,
...
class MenuPrivilegeBulkWizard(models.TransientModel):
    _name = 'menu.privilege.bulk.wizard.app'
    _description = 'Bulk Create Menu Privileges (Apps)'

    user_ids = fields.Many2many(
        'res.users',
        'menu_privilege_bulk_wizard_app_user_rel',
        'wizard_id', 'user_id',
        string='Users',
        domain=[('share', '=', False)],
        required=True,
    ...
    menu_ids = fields.Many2many(
        'ir.ui.menu',
        'menu_privilege_bulk_wizard_app_menu_rel',
        'wizard_id', 'menu_id',
        string='Menus',
        required=True,
    )
    ...
    def action_create_menu_privileges(self):
        self.ensure_one()
        MenuPrivilege = self.env['menu.privilege.app']
        for user in self.user_ids:
...
class ModuleVisibilityBulkWizard(models.TransientModel):
    _name = 'module.visibility.bulk.wizard.app'
    _description = 'Bulk Set Module (App) Visibility (Apps)'

    user_ids = fields.Many2many(
        'res.users',
        'module_visibility_bulk_wizard_app_user_rel',
        'wizard_id', 'user_id',
        string='Users',
        domain=[("share", "=", False)],
        required=True,
    ...
    module_ids = fields.Many2many(
        'ir.module.module',
        'module_visibility_bulk_wizard_app_mod_rel',
        'wizard_id', 'module_id',
        string='Modules (Apps)',
    ...
    def action_apply(self):
        self.ensure_one()
        ModuleVis = self.env['module.visibility.app']
        for user in self.user_ids:
