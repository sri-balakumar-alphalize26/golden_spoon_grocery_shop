...
def post_init_hook(env):
    ...
    """
    try:
        ModPriv = env['module.privilege.app'].sudo()
        UserPriv = env['user.privilege.app'].sudo()
        PrivRole = env['privilege.role.app'].sudo()
    ...
    try:
        env['module.privilege.app'].sudo().action_sync_all_to_user_privileges()
    except Exception:
        pass
    ...
    try:
        all_roles = env['privilege.role.app'].sudo().search([('active', '=', True)])
