...
class PrivilegeRole(models.Model):
    ...
    _name = 'privilege.role.app'
    _description = 'Privilege Group (Apps)'
    _order = 'name'
    ...
    odoo_group_ids = fields.Many2many(
        'res.groups',
        'privilege_role_app_odoo_groups_rel',
        'role_id', 'group_id',
    ...
    company_ids = fields.Many2many(
        'res.company',
        'privilege_role_app_company_rel',
        'role_id', 'company_id',
        string='Companies',
        default=lambda self: self.env.company,
    ...
    line_ids = fields.One2many(
        'role.privilege.line.app', 'role_id',
        string='Model Privileges',
        copy=True,
    )

    menu_visibility_ids = fields.One2many(
        'role.menu.visibility.app', 'role_id',
    ...
    module_visibility_ids = fields.One2many(
        'role.module.visibility.app', 'role_id',
    ...
    app_feature_visibility_ids = fields.One2many(
        'role.app.feature.visibility.app', 'role_id',
    ...
    add_model_ids = fields.Many2many(
        'ir.model',
        'privilege_role_app_add_model_rel',
        'role_id', 'model_id',
        string='Add Models',
    ...
    def _get_all_effective_groups(self):
        """Return all Odoo groups this privilege group grants."""
    ...
    def _get_auto_detected_groups(self):
        """Scan this group's model lines and return Odoo groups required."""
    ...
    def _apply_dependency_groups_to_users(self, module_rec, user_ids):
        """Check module's dependencies and add users to required Odoo groups."""
        if not user_ids:
            return
        dep_names = set()
    ...
    def _apply_visibility_to_users(self, user_ids):
        """When users are added, apply menu/module visibility restrictions."""
        MenuPrivilege = self.env['menu.privilege.app'].sudo()
        ModuleVis = self.env['module.visibility.app'].sudo()
    ...
    def _remove_visibility_from_users(self, user_ids):
        """When users are removed, clean up menu.privilege records."""
        MenuPrivilege = self.env['menu.privilege.app'].sudo()
            ...
            for modv in rec.module_visibility_ids.filtered(lambda x: not x.is_visible):
                ...
                for uid in user_ids:
                    if modv.module_name not in other_hidden_mods:
                        self.env['module.visibility.app'].sudo()._sync_menu_privileges(
    ...
    def action_load_from_groups(self):
        """Auto-populate Model Privileges from selected Odoo groups."""
        self.ensure_one()
        if not self.odoo_group_ids:
            return {
                'type': 'ir.actions.client',
    ...
    def action_load_menus_from_groups(self):
        """Auto-populate Menus tab from menus belonging to the selected Odoo groups."""
        self.ensure_one()
        if not self.odoo_group_ids:
            return {
                'type': 'ir.actions.client',
    ...
    def action_open_add_models_wizard(self):
        ...
        return {
            'name': 'Add Models',
            'type': 'ir.actions.act_window',
            'res_model': 'role.add.models.wizard.app',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_add_module_models(self, module_id):
        ...
        created = self.env['role.privilege.line.app'].sudo().create(new_lines) if new_lines else self.env['role.privilege.line.app']
    ...
    def action_add_module_menus(self, module_id):
        """Add all menus belonging to a module as role.menu.visibility entries."""
        self.ensure_one()
        module_rec = self.env['ir.module.module'].sudo().browse(module_id)
        if not module_rec.exists():
            return 0
        ...
        RoleMenuVis = self.env['role.menu.visibility.app'].sudo()
        created = 0
        for menu in menus:
    ...
    def get_role_privileges(self, model_name, user_id):
        ...
        lines = self.env['role.privilege.line.app'].sudo().search([
...
class RolePrivilegeLine(models.Model):
    ...
    _name = 'role.privilege.line.app'
    _description = 'Group Privilege Line (Apps)'
    _order = 'model_id'

    role_id = fields.Many2one('privilege.role.app', required=True, ondelete='cascade')
...
class RoleMenuVisibility(models.Model):
    ...
    _name = 'role.menu.visibility.app'
    _description = 'Group Menu Visibility (Apps)'
    ...
    def _push_to_role_users(self, is_visible):
        """Push this menu's visibility to all users in the group."""
        MenuPrivilege = self.env['menu.privilege.app'].sudo()
...
class RoleModuleVisibility(models.Model):
    ...
    _name = 'role.module.visibility.app'
    _description = 'Group Module (App) Visibility (Apps)'
    ...
    def _sync_module_privilege_lines(self):
        """Auto-create/update role.privilege.line entries for all models of this module."""
    ...
    def _push_to_role_users(self, is_visible):
        ...
        ModVis = self.env['module.visibility.app'].sudo()
        for user in self.role_id.user_ids:
...
class RoleAppFeatureVisibility(models.Model):
    ...
    _name = 'role.app.feature.visibility.app'
    _description = 'Group App Feature Visibility (Apps)'
