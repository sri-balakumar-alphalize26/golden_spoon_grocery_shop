...
class UserPrivilege(models.Model):
    _name = 'user.privilege.app'
    _description = 'User Privilege (Apps)'
    ...
    def get_user_privilege(self, model_name, user_id=None):
        ...
        if user.has_group('user_privilege_manager_apps.group_privilege_apps_manager'):
            return None
        ...
        role_privs = self.env['privilege.role.app'].get_role_privileges(model_name, user_id)
        if role_privs:
            return role_privs
        ...
        try:
            ...
            if model_rec:
                mp_line = self.env['module.privilege.line.app'].sudo().search([
                    ('user_id', '=', user_id),
